...
function removeItem(e) {
    if (e.target.classList.contains('delete')) {
        if (confirm('Are You Sure?')) {
            userItems = userItems.filter((item) => e.target.parentElement.firstChild.textContent != item)
            localStorage.setItem('items', userItems.join(','));
            var li = e.target.parentElement;
            itemList.removeChild(li);
            visible = Array.from(document.querySelectorAll('.list-group-item')).filter(e => { return e.style.display != 'none' });
        }
    }
}
...
function checkKey(e) {
    ...
    let visible = Array.from(document.querySelectorAll('.list-group-item')).filter(e => { return e.style.display != 'none' });
    if (selected) {
        ...
        if (e.keyCode == '38') {
            ...
            visible[selectedIndex == 0 ? visible.length - 1 : --selectedIndex].classList.add(...selectClasses);
        }
        else if (e.keyCode == '40') {
            // DOWN
            visible[selectedIndex].classList.remove(...selectClasses);
            visible[selectedIndex == (visible.length - 1) ? 0 : ++selectedIndex].classList.add(...selectClasses);
